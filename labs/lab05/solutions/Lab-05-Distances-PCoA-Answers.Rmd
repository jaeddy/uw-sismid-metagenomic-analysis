---
title: "Lab 05: Ecological Distance Metrics and Principal Coordinates Analysis -- Example Solutions"
...
## PCoA
library(ade4)
test.pco <- dudi.pco(jsd.fecal, scannf = TRUE, full = TRUE)
screeplot(test.pco)
```
...
s.value(fecal.pco$li, sample_data(fecal.phy)$GIP, sub="GIP")
s.value(fecal.pco$li, sample_data(fecal.phy)$Insulin, sub="Insulin")
s.value(fecal.pco$li, sample_data(fecal.phy)$Leptin, sub="Leptin")
s.value(fecal.pco$li, sample_data(fecal.phy)$IGF1, sub="IGF1")
s.value(fecal.pco$li, sample_data(fecal.phy)$BMD, sub="BMD")
s.value(fecal.pco$li, sample_data(fecal.phy)$mFat, sub="mFat")
s.value(fecal.pco$li, sample_data(fecal.phy)$pFat, sub="pFat")
...
jsd.test.pco <- dudi.pco(jsd, scannf = F)
screeplot(jsd.test.pco)
```
...
jsd.wca = wca(jsd.pco, 
              sample_data(phy)$Location, scannf = F)
